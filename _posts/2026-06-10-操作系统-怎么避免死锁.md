...
那么，当两个线程为了保护两个不同的共享资源而使用了两个互斥锁，那么这两个互斥锁应用不当的时候，可能会造成**两个线程都在等待对方释放�?* ，在没有外力的作用下，这些线程会一直相互等待，就没办法继续运行，这种情况就是发生了**死锁** �?
...
互斥条件是指**多个线程不能同时使用同一个资�?* �?

比如下图，如果线�?A 已经持有的资源，不能再同时被线程 B 持有，如果线�?B 请求获取线程 A 已经占用的资源，那线�?B 只能等待，直到线�?A 释放了资源�?

![](https://aka.doubaocdn.com/s/SC351wZOxR)

### 持有并等待条�?

持有并等待条件是指，当线�?A 已经持有了资�?1，又想申请资�?2，而资�?2 已经被线�?C 持有了，所以线�?A 就会处于等待状态，但是**线程 A 在等待资�?2 的同时并不会释放自己已经持有的资�?1** �?

![](https://aka.doubaocdn.com/s/bq7f1wZOxR)

### 不可剥夺条件

不可剥夺条件是指，当线程已经持有了资�?�?*在自己使用完之前不能被其他线程获�?* ，线�?B 如果也想使用此资源，则只能在线程 A 使用完并释放后才能获取�?

![](https://aka.doubaocdn.com/s/9wOx1wZOxR)

### 环路等待条件

环路等待条件指的是，在死锁发生的时候，**两个线程获取资源的顺序构成了环形�?* �?

比如，线�?A 已经持有资源 2，而想请求资源 1�?线程 B 已经获取了资�?1，而想请求资源 2，这就形成资源请求等待的环形图�?

![](https://aka.doubaocdn.com/s/IhhF1wZOxR)
...
首先，我们先创建 2 个线程，分别为线�?A �?线程 B，然后有两个互斥锁，分别�?mutex_A �?mutex_B，代码如下：
...
如果你想排查你的 Java 程序是否死锁，则可以使用 `jstack` 工具，它�?jdk 自带的线程堆栈分析工具�?

由于小林的死锁代码例子是 C 写的，在 Linux 下，我们可以使用 `pstack` + `gdb` 工具来定位死锁问题�?

pstack 命令可以显示每个线程的栈跟踪信息（函数调用过程），它的使用方式也很简单，只需�?`pstack <pid>` 就可以了�?
...
#1 0x0000000000400806 in main ()
```
可以看到，Thread 2 �?Thread 3 一直阻塞获取锁�?pthread_mutex_lock*）的过程，而且 pstack 多次输出信息都没有变化，那么可能大概率发生了死锁�?
...
// 打印第三帧信息，每次函数调用都会有压栈的过程，�?frame 则记录栈中的帧信�?
(gdb) frame 3
#3 0x0000000000400792 in threadB_proc (data=0x0) at dead_lock.c:25
27 printf("thread B waiting get ResourceA \n");
28 pthread_mutex_lock(&mutex_A);

// 打印mutex_A的�?, __owner表示gdb中标示线程的值，即LWP
(gdb) p mutex_A
...
// 打印mutex_B的�?, __owner表示gdb中标示线程的值，即LWP
(gdb) p mutex_B
...
1. 通过 `info thread` 打印了所有的线程信息，可以看到有 3 个线程，一个是主线程（LWP 87746），另外两个都是我们自己创建的线程（LWP 87747 �?87748）；

2. 通过 `thread 2`，将切换到第 2 个线程（LWP 87748）；

3. 通过 `bt`，打印线程的调用栈信息，可以看到�?threadB_proc 函数，说明这个是线程 B 函数，也就说 LWP 87748 是线�?B;

4. 通过 `frame 3`，打印调用栈中的第三个帧的信息，可以看到线程 B 函数，在获取互斥�?A 的时候阻塞了�?

5. 通过 `p mutex_A`，打印互斥锁 A 对象信息，可以看到它�?LWP �?87747（线�?A�?的线程持有着�?

6. 通过 `p mutex_B`，打印互斥锁 B 对象信息，可以看到他�?LWP �?87748 （线�?B�?的线程持有着�?

因为线程 B 在等待线�?A 所持有�?mutex_A, 而同时线�?A 又在等待线程 B 所拥有的mutex_B, 所以可以断定该程序发生了死锁�?

## 避免死锁问题的发�?

前面我们提到，产生死锁的四个必要条件是：互斥条件、持有并等待条件、不可剥夺条件、环路等待条件�?

那么避免死锁问题就只需要破坏其中一个条件就可以，最常见的并且可行的就是**使用资源有序分配法，来破坏环路等待条�?* �?

那什么是资源有序分配法呢�?

线程 A �?线程 B 获取资源的顺序要一样，当线�?A 是先尝试获取资源 A，然后尝试获取资�?B 的时候，线程 B 同样也是先尝试获取资�?A，然后尝试获取资�?B。也就是说，线程 A �?线程 B 总是以相同的顺序申请自己想要的资源�?

我们使用资源有序分配法的方式来修改前面发生死锁的代码，我们可以不改动线程 A 的代码�?

我们先要清楚线程 A 获取资源的顺序，它是先获取互斥锁 A，然后获取互斥锁 B�?

所以我们只需将线�?B 改成以相同顺序的获取资源，就可以打破死锁了�?
...
![](https://aka.doubaocdn.com/s/sYOL1wZOxR)


 [  5.5 什么是悲观锁、乐观锁�? ](https://xiaolincoding.com/os/4_process/pessim_and_optimi_lock.html)  �?

---
> 参考来源：[怎么避免死锁？](https://xiaolincoding.com/os/4_process/deadlock.html)
