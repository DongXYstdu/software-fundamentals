...
* **规则一：必须从最左列开始** 。 `WHERE b = 2` 缺少最左列 `a` ，无法利用索引的有序性，只能全表扫描。
* **规则二：中间不能跳过** 。 `WHERE a = 1 AND c = 3` 跳过了 `b` ，只能使用 `a` 列的索引， `c` 无法走索引。
* **规则三：范围查询会 "断后"** 。一旦出现范围查询（ `>` 、 `<` 、 `BETWEEN` 、 `LIKE 'x%'` ），该列之后的索引列无法被使用。
...
* **OR 条件的陷阱** ： `OR` 条件通常导致全表扫描，因为无法保证所有条件都能走索引。
