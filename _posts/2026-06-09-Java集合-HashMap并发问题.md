...
ConcurrentHashMap 在 JDK 1.8 中采用 CAS + 桶级 synchronized 的方式保证线程安全：

- **初始化数组**：使用 CAS 保证只有一个线程初始化
- **插入新节点**：使用 CAS 无锁操作
- **更新已有节点**：使用 synchronized 锁住链表头节点或红黑树根节点
- **可见性**：使用 volatile 保证节点值和 next 指针的可见性
