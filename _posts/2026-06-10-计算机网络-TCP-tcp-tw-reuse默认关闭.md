...
但是，在看了防回绕序列号函数的源码后，发现对于 **RST 报文的时间戳即使过期了，只要 RST 报文的序列号在对方的接收窗口内，也是能被接受的**。
...
处于 syn_sent 状态的客户端收到服务端的 Challenge ACK 后，发现不是自己期望收到的确认号，于是就会回复 RST 报文，服务端收到后就会释放连接。

所以开启 tcp_tw_reuse 后，如果第四次挥手的 ACK 报文丢失了，那么客户端和服务端都需要经历一些额外的交互才能关闭旧连接，虽然最终能关闭，但是这个过程需要耗费一些时间。
