...
你点开后发现，长相平平无奇的古某提示你"道士 9 条狗，全服横着走"。
...
- 在 HTTP/1.1 里，只要客户端不问，服务端就不答。基于这样的特点，对于登录页面这样的简单场景，可以使用**轮询**或**长轮询**来实现伪服务器推送，但对于需要频繁交互的场景（如网页游戏），这些方式就不太合适了。

- WebSocket 是一个**全双工**的应用层协议，它通过 HTTP 协议升级建立连接，建立之后就和 HTTP 没有关系了。它适用于需要服务器和客户端频繁交互的场景，比如网页游戏、聊天室、协同办公等。

---
> 参考来源：[既然有 HTTP 协议，为什么还要有 WebSocket？](https://xiaolincoding.com/network/2_http/http_websocket.html)
