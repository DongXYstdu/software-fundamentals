...
- **BigDecimal 如何保证精度�?* `BigDecimal` 通过一�?**`BigInteger` 类型的非标度值（`unscaledValue`�?* 和一�?**`int` 类型的标度（`scale`�?* 来表示一个数。其值为 `unscaledValue × 10^(-scale)`。这种基于十进制的表示法，使其在金融计算中（如加减乘除）可以完全避免二进制浮点数（如 `0.1` 在二进制中是循环小数）带来的精度问题。此外，它允许你精确控制舍入行为（通过 `RoundingMode`）�?

- **Long 如何表示金额�?* 使用 `Long` 时，金额的物理单位并�?"�?，而是选择一�?**"缩放因子（Scaling Factor�?**，通常是该货币的最小单位，如人民币�?**"�?**。例如，`123.45 元` 在数据库和内存中存储�?`12345`（`Long` 类型）。所有计算都在这个放大后的整数基础上进行，仅在最终展示给用户时，才除以缩放因子转换为"�?。这本质上是一�?**"定点�?** 的模拟�?
