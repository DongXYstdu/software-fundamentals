...
| 特?| 说明 |
|---|---|
| 运行时类型检?| 程序运行时获取对象所属的类信?|
| 动态加载类 | 运行时加载编译期间未知的?|
| 动态调用方?| 运行时调用任意对象的方法 |
| 访问私有成员 | 突破封装限制，访?`private` 成员 |
...
| 参数装箱拆箱 | 基本类型需要包装成 `Object[]` 传?|
| 安全检?| 每次调用都要检查访问权限（`setAccessible` 可绕过） |
| JIT 优化受限 | 反射调用难以?JIT 内联优化 |
...
![](https://aka.doubaocdn.com/s/sipi1wZNgo)

上图展示?Java 反射机制的核心组成。整个反射体系的工作原理可以分为以下几个层次?

- **编译阶段**：`.java` 源文件被 `javac` 编译?`.class` 字节码文件，包含了类的完整元数据信息?

- **类加载阶?*：类加载器将 `.class` 文件加载?JVM 中，在方法区（JDK 8 之后是元空间）生成唯一?`Class` 对象，作为访问该类所有信息的入口?

- **Class 对象**：每个类?JVM 中都有且仅有一?`Class` 对象，它存储了类的所有元数据，包括类名、修饰符、父类、接口、字段数组、方法数组、构造器数组等?

- **反射操作**：通过 `Class` 对象，可以在运行时获取类的任意信息、创建实例、调用方法、访问字段，突破编译时的限制?

关键点在于，**反射的本质是操作方法区中?`Class` 对象**，?`Class` 对象在类加载时就已经准备好了所有元数据，反射只是提供了一?API 来访问这些数据?
...
| 方式 | 使用场景 | 类加载时?| 性能 |
|---|---|---|---|
| `类名.class` | 编译时已知具体类 | 使用时才加载（延迟加载） | 最?|
| `对象.getClass()` | 已有对象实例 | 对象创建时已加载 | ?|
| `Class.forName()` | 配置驱动、框架开?| 立即加载并初始化 | 较低 |
...
![](https://aka.doubaocdn.com/s/BfDv1wZNgo)
...
反射需要在运行时遍历类的方法数组，逐个对比方法名和参数类型，这是一?**O(n)** 复杂度的查找过程?
...
每次反射调用都会检?`accessFlags`，判断调用者是否有权限访问该成员。调?`setAccessible(true)` 可以绕过检查，但也破坏了封装性?
...
// 反射调用：JIT 难以预测目标方法，无法内?
method.invoke(user, "hi");
// 只能通过 JNI ?MethodAccessor 调用，无法优?
```
...
// 典型输出结果?
// 直接调用?20 ms
// 反射调用?500 ms（约 30 倍差距）
// 反射调用（优化）?800 ms（约 15 倍差距）
```
...
| 场景 | 说明 | 示例 |
|---|---|---|
| 框架开?| 解耦配置与代码，动态加载类 | Spring IOC、MyBatis |
| 动态代?| 运行时生成代理类 | JDK 动态代理、AOP |
| 注解处理 | 运行时读取注解信?| `@Autowired`、`@Controller` |
| 序列化框?| 动态访问对象字?| Gson、Jackson、Fastjson |
...
2. **`Class.forName()` ?`ClassLoader.loadClass()` 有什么区别？**
    ...
    - `Class.forName()` 会加载类并执行静态初始化?
    - `ClassLoader.loadClass()` 只加载类，不执行初始?
    - JDBC 使用 `Class.forName()` 就是为了触发驱动的静态注?

3. **如何避免反射的性能损耗？**

    - 缓存 `Method`、`Field` 对象
    - 调用 `setAccessible(true)` 关闭安全检?
    - 高频场景使用 `MethodHandle` 或字节码生成技?
...
**反射三要?*：`Class` 对象是入口，`Method` 调用是核心，`setAccessible` 突破限制?
...
Java 反射机制允许程序在运行时获取类的完整信息并动态调用方法，是框架开发的基石。反射慢的根本原因是运行时方法查找、参数装箱、安全检查和 JIT 优化受限，性能损耗约 10-100 倍。优化策略包括缓?`Method` 对象、调?`setAccessible(true)`、使?`MethodHandle` 或字节码生成技术?

---
> 参考来源：[什么是 Java 反射机制？为什么反射慢？](https://www.quanxiaoha.com/java-interview/what-is-java-reflection-why-slow)
