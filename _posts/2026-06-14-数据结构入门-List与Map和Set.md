...
不同的需求用不同的工具。编程也一样——**List、Map、Set** 就是三种最常用的"工具箱"，选对了，代码又快又简洁；选错了，麻烦不断。
...
想象一个演唱会入场名单：**张三、李四、张三、王五**。注意，张三出现了两次，而且顺序很重要——先到先入场。
...
| **头部插入删除** | 🐢 慢 O(n) | ⚡ 快 O(1) |
| **内存** | 连续空间 | 分散空间 |
| **适用场景** | 查询多、尾部增删 | 频繁在头部或中间增删 |

> 💡 **简单记忆**：ArrayList = 数组包装，查询快；LinkedList = 链表包装，插入快。日常开发90%用ArrayList就够了。
...
public class ListDemo {
    public static void main(String[] args) {
        ...
        // 查：按位置访问
        System.out.println("第1个人：" + nameList.get(0));  // 张三
...
# 删
name_list.remove("张三")  # 删除第一个
del name_list[0]          # 按位置删除
```
...
HashMap 就像一个**超级通讯录**，通过一种叫"哈希"的魔法，能在海量数据中瞬间找到你要的条目。

> 💡 **简单理解哈希**：就像图书馆的编号系统，根据书名算出一个编号，直接去对应书架找书，不用一本一本翻。
...
public class MapDemo {
    public static void main(String[] args) {
        ...
        // 改：键相同则覆盖
        contacts.put("妈妈", "138xxxx9999");  // 妈妈换号了
...
print(contacts.get("同学", "未找到"))  # 未找到（安全访问）
...
| **生活类比** | 排队名单 | 通讯录 | 抽奖箱 |
| **是否有序** | ✅ 有序 | ❌ 键无序 | ❌ 无序 |
| **是否可重复** | ✅ 可重复 | ❌ 键不可重复 | ❌ 不可重复 |
| **查找速度** | 🐢 O(n) 逐个找 | ⚡ O(1) 按键瞬间找 | ⚡ O(1) 瞬间判断 |
| **访问方式** | 按位置 get(index) | 按键 get(key) | 判断存在 contains() |
| **典型场景** | 保存顺序、允许重复 | 一一对应关系 | 去重、判断存在 |
...
graph TD
    ...
    Q1 -->|是| MAP["用 Map<br/>如 HashMap"]
    Q1 -->|否| Q2{"需要去重吗？<br/>（同一元素只留一个）"}
    Q2 -->|是| SET["用 Set<br/>如 HashSet"]
    Q2 -->|否| Q3{"需要保持顺序且允许重复？"}
    Q3 -->|是| LIST["用 List<br/>如 ArrayList"]
    Q3 -->|否| Q4{"只是判断存在？"}
    Q4 -->|是| SET
    Q4 -->|否| LIST
...
| 实现类 | key为null | value为null |
|:---:|:---:|:---:|
| HashMap | ✅ 允许一个null键 | ✅ 允许多个null值 |
| TreeMap | ❌ 不允许（要排序） | ❌ 不允许 |
| ConcurrentHashMap | ❌ 不允许 | ❌ 不允许 |

> ⚠️ 建议：虽然HashMap允许null键，但**最好不要用null作为key**，容易出bug且难以调试。

### 陷阱2：Set怎么判断重复？

Set判断两个元素是否"相同"，不是用 `==`，而是用 **`equals()` + `hashCode()`**：
...
> 💡 **安全做法**：如果需要独立的子列表，用 `new ArrayList<>(list.subList(1, 3))` 创建副本。
...
graph TB
    ...
    COL --> LIST["List<br/>有序·可重复<br/>排队名单"]
    COL --> SET_NODE["Set<br/>无序·不重复<br/>抽奖箱"]

    LIST --> AL["ArrayList<br/>数组实现·查询快"]
    LIST --> LL["LinkedList<br/>链表实现·插入快"]

    SET_NODE --> HS["HashSet<br/>哈希实现·最常用"]
    SET_NODE --> TS["TreeSet<br/>红黑树实现·有序"]

    MAP_NODE["Map<br/>键值对·一键一值<br/>通讯录"] --> HM["HashMap<br/>哈希实现·最常用"]
    MAP_NODE --> TM["TreeMap<br/>红黑树实现·按键排序"]
...
- **Map = 通讯录**：键值对，按键查找极快，适合一对一映射
- **Set = 抽奖箱**：不重复，判断存在极快，适合去重

选容器的口诀：**映射用Map，去重用Set，排队用List**。

下一篇文章，我们将深入C语言，学习如何用**结构体**来组织复杂的数据——这是理解数据结构底层实现的关键一步！
