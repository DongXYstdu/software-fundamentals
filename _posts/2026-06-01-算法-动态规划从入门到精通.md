...
| 性质 | 含义 | 判断方法 |
|:-----|:-----|:---------|
| **最优子结构** | 问题的最优解包含子问题的最优解 | 假设子问题已最优，能否推出原问题最优 |
| **重叠子问题** | 递归求解时同一子问题被反复计算 | 画出递归树，观察是否有重复节点 |
...
| 特征 | 动态规划 | 贪心 | 分治 |
|:-----|:---------|:-----|:-----|
| 子问题独立性 | 子问题重叠 | 无回溯 | 子问题独立 |
| 最优解保证 | ✅ 全局最优 | ❌ 局部最优 | ✅ 合并即最优 |
| 典型问题 | 背包、LCS | 活动选择、Huffman | 归并排序、FFT |
| 复杂度 | 通常多项式 | 通常线性 | 通常 $O(n \log n)$ |
...
> **空间优化核心思想**：如果 $dp[i]$ 只依赖前 $k$ 个状态，就可以用滚动变量代替整个数组。
{: .prompt-tip}

### 3.2 0-1 背包问题

**问题描述**：有 $n$ 个物品，重量 $w_i$，价值 $v_i$，背包容量 $W$，每个物品最多选一次，求最大价值。

**状态定义**：$dp[i][j]$ = 从前 $i$ 个物品中选，容量为 $j$ 时的最大价值
...
| 类型 | 每个物品 | 内层循环方向 | 状态转移 |
|:-----|:--------:|:----------:|:---------|
| 0-1 背包 | 选0或1次 | 逆序 | $dp[j] = \max(dp[j], dp[j-w]+v)$ |
| 完全背包 | 选任意次 | 正序 | $dp[j] = \max(dp[j], dp[j-w]+v)$ |
| 多重背包 | 选有限次 | 二进制拆分 | 转化为多个0-1背包 |

### 3.3 最长公共子序列（LCS）

**问题描述**：给定两个序列，求它们最长的公共子序列长度。

**状态定义**：$dp[i][j]$ = $s_1$ 前 $i$ 个字符与 $s_2$ 前 $j$ 个字符的 LCS 长度
...
**输出具体 LCS**：通过回溯 $dp$ 表，从 $dp[m][n]$ 向左上追踪。
...
> **tails 数组的含义**：维护各个长度 LIS 的最小末尾元素。tails 一定是严格递增的，因此可以用二分查找。
{: .prompt-info}

### 3.5 编辑距离（Edit Distance）

**问题描述**：将 $s_1$ 转换为 $s_2$ 所需的最少操作次数（插入、删除、替换）。

**状态定义**：$dp[i][j]$ = $s_1$ 前 $i$ 个字符变为 $s_2$ 前 $j$ 个字符的最少操作数
...
**问题描述**：给定面额 $coins$ 和金额 $amount$，求凑出 $amount$ 的最少硬币数。

**状态定义**：$dp[j]$ = 凑出金额 $j$ 的最少硬币数

**状态转移**：$dp[j] = \min_{c \in coins}(dp[j-c] + 1)$，若 $j \ge c$
...
> **零钱兑换是"完全背包"的变体**：每种硬币可无限使用，求最小值而非最大值。
{: .prompt-tip}
...
| 模式 | 状态定义 | 典型问题 |
|:-----|:---------|:---------|
| 线性 DP | $dp[i]$ = 以位置 $i$ 结尾的最优值 | LIS、最大子数组和 |
| 区间 DP | $dp[i][j]$ = 区间 $[i,j]$ 的最优值 | 矩阵链乘法、戳气球 |
| 背包 DP | $dp[i][j]$ = 前 $i$ 个物品容量 $j$ 的最优值 | 0-1背包、完全背包 |
| 树形 DP | $dp[u][s]$ = 节点 $u$ 状态 $s$ 的最优值 | 树的最大独立集 |
| 状态压缩 DP | $dp[s]$ = 集合状态 $s$ 的最优值 | TSP、棋盘覆盖 |

### 4.3 区间 DP 示例：矩阵链乘法

**状态转移**：$dp[i][j] = \min_{i \le k < j}(dp[i][k] + dp[k+1][j] + p_{i-1} \cdot p_k \cdot p_j)$
...
当 $dp[i]$ 只依赖 $dp[i-1]$（或前几行），可以用两个数组交替使用。
...
用二进制表示集合状态，常用于 NP-hard 问题的精确求解。
...
2. **重叠子问题**：递归树中是否有大量重复计算

如果只有最优子结构没有重叠子问题 → 分治；如果贪心选择性质成立 → 贪心。
...
两种方法：
1. **回溯法**：从最终状态沿转移路径回溯
2. **记录法**：额外维护 $choice$ 数组，记录每步的决策

**Q5：编辑距离的三种操作分别对应什么？**

- $dp[i-1][j-1] + 1$：替换 $s_1[i]$ 为 $s_2[j]$
- $dp[i-1][j] + 1$：删除 $s_1[i]$
- $dp[i][j-1] + 1$：在 $s_1$ 的位置 $i$ 插入 $s_2[j]$

## 8. 总结

动态规划的精髓在于**状态定义**和**状态转移方程**。掌握常见模式（线性、区间、背包、树形、状压），辅以大量练习，才能在面试和竞赛中游刃有余。
