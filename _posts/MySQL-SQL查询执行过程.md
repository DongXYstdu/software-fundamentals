...
第二种， **客户端主动重置连接** 。MySQL 5.7 版本实现了 `mysql_reset_connection()` 函数的接口，注意这是接口函数不是命令，那么当客户端执行了一个很大的操作后，在代码里调用 mysql\_reset\_connection 函数来重置连接，达到释放内存的效果。这个过程不需要重连和重新做权限验证，但是会将连接恢复到刚刚创建完时的状态。
...
上面的中间部分是 MySQL 报错表不存在时的函数调用栈，可以看到表不存在的错误是在get\_table\_share() 函数里报错的，而这个函数是在 prepare 阶段调用的。
...
<a class='context-link prev' href='/software-fundamentals/posts/MySQL-MySQL面试题/'><span class='context-label'>上一篇</span><span class='context-title'>MySQL面试</span></a>
<a class='context-link next' href='/software-fundamentals/posts/MySQL-count区别/'><span class='context-label'>下一篇</span><span class='context-title'>count(*)和count(1)区别</span></a>
</div>
