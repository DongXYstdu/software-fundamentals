...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144822785.jpeg)

数据表越大，磁盘上的 ibd 文件也就越大。

直接读写一个大文件里的全部数据会很慢，所以 MySQL 将数据 **拆成一个个数据页** ，每页大小 **16KB** 。这样我们读写部分表数据的时候，就只需要读取磁盘里的 **几个数据页** 就好。

![](https://cdn.xiaolincoding.com//picgo/640-20260131144826081.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144829312.jpeg)

上面提到的是针对主键的索引，也就是 **主键索引** 。

![](https://cdn.xiaolincoding.com//picgo/640-20260131144832145.jpeg)

按同样的思路，也可以为 **其他数据表的列** 去建立 **索引** ，比如用户表的名称字段，这样我们就能快速查找到名字为 xx 的用户有哪些，这就是所谓的 **辅助索引** 。

![](https://cdn.xiaolincoding.com//picgo/640-20260131144835435.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144838881.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144842430.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144846121.jpeg)

这个 hash 表，就是所谓的 **自适应哈希索引** ， **Adaptive Hash Index** 。

![](https://cdn.xiaolincoding.com//picgo/640-20260131144849033.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144852674.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144855713.jpeg)

## Undo Log

在数据库中，有一个叫 **事务** 的概念。不了解没关系，说白了，就是可以让多行数据，要么 **同时** 更新成功，要么 **同时** 更新失败。也就是所谓的 **原子性** 。

![](https://cdn.xiaolincoding.com//picgo/640-20260131144858618.jpeg)

为了实现这一点，我们就需要知道写数据时每行数据 **原来长啥样** ，方便对更新后的数据行，进行 **回滚** ，因此就有了 **Undo Log** 。

![](https://cdn.xiaolincoding.com//picgo/640-20260131144901378.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144904270.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144908055.jpeg)

这时候问题就来了，我有这功夫更新 redo log file 文件，直接将 buffer pool 的数据写入到磁盘不香吗？

![](https://cdn.xiaolincoding.com//picgo/640-20260131144911475.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144914573.jpeg)
...
比如操作数据行的 write\_row(), update\_row()，以及操作数据表的 create(), drop()等等接口。

我们平时写的 SQL 语句，最终都会转换成 InnoDB 提供的这些接口函数调用。

![](https://cdn.xiaolincoding.com//picgo/640-20260131144918346.jpeg)

比如：

* INSERT 语句会调用 write\_row() 接口来插入数据行。
* UPDATE 语句会调用 update\_row() 接口来更新数据行。
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144922410.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144925469.jpeg)

**server 层** 和 **存储引擎层** 共同构成了一个完整的数据库，它就是我们常说的 **Mysql 数据库** 。

![](https://cdn.xiaolincoding.com//picgo/640-20260131144928439.jpeg)

并且，server 层和存储引擎层是通过接口函数进行 **解耦** 的，换句话说就是，只要实现了上面这些接口函数，就能作为存储引擎与 Server 层对接。

![](https://cdn.xiaolincoding.com//picgo/640-20260131144931330.jpeg)

比如，mysql 早期用的是 **myisam** 存储引擎，后来才支持的 **innodb** 。

![](https://cdn.xiaolincoding.com//picgo/640-20260131144933803.jpeg)
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144937556.jpeg)
...
接下来我们用实际例子将上面提到的内容串起来。首先不管是查询还是更新操作，客户端都会先跟 mysql 建立网络连接，并将 sql 发送到 **server 层** ，经过 **分析器** 解析 sql 语法、 **优化器** 选择索引生成执行计划，最终给到 **执行器** 调用 InnoDB 的函数接口。
...
![](https://cdn.xiaolincoding.com//picgo/640-20260131144941462.jpeg)
...
* innodb 分为内存和磁盘两部分，一部分是内存里的自适应 hash，buffer pool，以及 redo log buffer。另一部分是磁盘里存放行数据和索引的.ibd 文件, 以及 undo log, redo log 等文件。
...
<a class='context-link prev' href='/software-fundamentals/posts/MySQL-MySQL加锁机制/'><span class='context-label'>上一篇</span><span class='context-title'>MySQL是怎么加锁的？</span></a>
<a class='context-link next' href='/software-fundamentals/posts/MySQL-MySQL死锁处理/'><span class='context-label'>下一篇</span><span class='context-title'>MySQL死锁了怎么办？</span></a>
</div>
