...
- **MyISAM**：在表元数据中存储了精确的行数，`COUNT(*)` 直接读取即可，O(1) 复杂度
- **InnoDB**：由于 MVCC 机制，不同事务看到的行数可能不同，无法存储统一的计数，需要扫描索引统计，O(N) 复杂度
...
---
> 参考来源：[MySQL 中 count(1)、count(\*) 与 count(列名) 的区别？](https://www.quanxiaoha.com/java-interview/mysql-count-star-count-1-count-column-difference)
