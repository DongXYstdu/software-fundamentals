...
**核心原则** ：联合索引要遵循 "最左前缀原则"，索引列的顺序非常重要。将 **区分度高、经常用于查询条件** 的列放在左边。
...
3. **如何判断索引是否生效？**
   ...
   使用 `EXPLAIN` 查看执行计划，检查 `key` 字段（实际使用的索引）和 `type` 字段（访问类型）。 `type` 为 `ref` 、 `range` 、 `const` 等表示索引生效， `ALL` 表示全表扫描。
