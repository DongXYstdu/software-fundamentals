...
# [#](#spring面试�? Spring面试�?[![image](https://cdn.xiaolincoding.com/mianshiya.png)](https://mianshiya.com/?shareCode=xeu1wi)

 
---
 大家好，我是小林�?

 Spring 这块�?Java 后端面试里的地位毋庸置疑，几乎是必考内容。不管是校招还是社招，IOC、AOP、事务、Bean 生命周期这些主题，基本每次都逃不掉。很多人平时天天�?Spring 写代码，@Autowired 一打、@Transactional 一加就完事了，但真被问�?IOC 底层怎么实现�?"循环依赖是怎么解决�?"事务为什么会失效"，就开始语焉不详了�?

 这篇文章整理�?Spring 全家桶面试中最常被问到的知识点，涵�?Spring 核心（IOC/AOP/事务/Bean）、SpringMVC、SpringBoot 自动装配、MyBatis，以�?SpringCloud 微服务组件，内容覆盖比较全面�?

 几个在面试里出现频率特别高的方向，建议重点花时间�?

 - **IOC �?AOP**：IOC 的底层实现原理、三种依赖注入方式的区别，AOP 的动态代理机制（JDK 动态代�?vs CGLIB），以及 AOP 的实际应用场景，这两块是 Spring 面试的绝对核心�?
 - **循环依赖和三级缓�?*：为什么需要三级缓存、二级缓存为什么不够，这个问题在面试里被问得越来越频繁，而且很容易被追问到细节�?
 - **Spring 事务**：事务什么时候会失效（用 this 调用、非 public 方法、异常类型不对），这是一道经典坑题，答起来很容易只说表面�?
 - **SpringBoot 自动装配原理**：`@SpringBootApplication` 背后的逻辑、`spring.factories` 是怎么工作的，这块�?SpringBoot 的核心，面试里问的很多�?
 - **Bean 的生命周�?*：从实例化到销毁经历了哪些步骤、各个扩展点在哪里介入，这块答好了能体现�?Spring 底层的理解深度�?
 如果你是第一次系统准�?Spring，建议先搞清�?IOC �?AOP 的原理，再去�?Bean 生命周期和循环依赖，顺序理清楚之后，后面�?SpringBoot 自动装配理解起来会顺很多�?

 
---
 ## [#](#spring) Spring ### [#](#说一下你�?spring-的理�? 说一下你�?Spring 的理�?![image](https://cdn.xiaolincoding.com//picgo/1712650311366-b499469c-5afd-4be9-bad3-d787de86bf98.png)
 ...
 ### [#](#spring的核心思想说说你的理解) spring的核心思想说说你的理解�?**
**核心思想**** | ****解决的问�?*** | ****实现手段**** | ****典型应用场景**** | 
**IOC** | 对象创建与依赖管理的高耦合 | 容器管理Bean生命周期 | 动态替换数据库实现、服务组�?| 
**DI** | 依赖关系的硬编码问题 | Setter/构造器/注解注入 | 注入数据源、服务层依赖DAO�?| 
**AOP** | 横切逻辑分散在业务代码中 | 动态代理与切面配置 | 日志、事务、权限校验统一处理 | Spring通过这IOC、DI、AOP三大核心思想，实现了轻量级、高内聚低耦合的企业级应用开发框架，成为Java生态中不可或缺的基石�?
 ...
 ### [#](#spring的aop介绍一�? Spring的aop介绍一�?Spring AOP是Spring框架中的一个重要模块，用于实现面向切面编程�?

 我们知道，Java 就是一门面向对象编程的语言，在 OOP 中最小的单元就是“Class 对象”，但是�?AOP 中最小的单元是“切面”。一个“切面”可以包含很多种类型和对象，对它们进行模块化管理，例如事务管理�?

 在面向切面编程的思想里面，把功能分为两种

 - **核心业务**：登陆、注册、增、删、改、查、都叫核心业�?
 - **周边功能**：日志、事务管理这些次要的为周边业�?
 在面向切面编程中，核心业务功能和周边功能是分别独立进行开发，两者不是耦合的，然后把切面功能和核心业务功能 "编织" 在一起，这就叫AOP�?

 AOP能够将那些与业务无关�?*却为业务模块所共同调用的逻辑或责任（例如事务处理、日志管理、权限控制等）封装起�?*，便�?*减少系统的重复代�?*�?*降低模块间的耦合�?*，并**有利于未来的可拓展性和可维护�?*�?

 �?AOP 中有以下几个概念�?

 - **Aspect**：切面，只是一个概念，没有具体的接口或类与之对应，�?Join point，Advice �?Pointcut 的一个统称。需要注意的是，Aspect �?切面"这一概念本身，�?AspectJ 是一个独立的 AOP 框架，Spring AOP 只是借用了它的注解（�?`@Aspect`）�?
 - **Join point**：连接点，指程序执行过程中的一个点，例如方法调用、异常处理等。在 Spring AOP 中，仅支持方法级别的连接点�?
 - **Advice**：通知，即我们定义的一个切面中的横切逻辑，包�?`before`、`after`、`after-returning`、`after-throwing`、`around` 五种类型。在很多�?AOP 实现框架中，Advice 通常作为一个拦截器，也可以包含许多个拦截器作为一条链路围绕着 Join point 进行处理�?
 - **Pointcut**：切点，用于匹配连接点，一�?AspectJ 中包含哪�?Join point 需要由 Pointcut 进行筛选�?
 - **Introduction**：引介，让一个切面可以声明被通知的对象实现任何他们没有真正实现的额外的接口。例如可以让一个代理对象代理两个目标类�?
 - **Weaving**：织入，在有了连接点、切点、通知以及切面，如何将它们应用到程序中呢？没错，就是织入，在切点的引导下，将通知逻辑插入到目标方法上，使得我们的通知逻辑在方法调用时得以执行�?
 - **AOP proxy**：AOP 代理，指�?AOP 实现框架中实现切面协议的对象。在 Spring AOP 中有两种代理，分别是 JDK 动态代理和 CGLIB 动态代理�?
 - **Target object**：目标对象，就是被代理的对象�?
 Spring AOP 是基�?JDK 动态代理和 CGLIB 字节码增强实现的，两种代理方式都属于**运行期织�?*，不需要专用编译器（与 AspectJ 的编译期 / 类加载期织入相对），因此 Spring AOP 完全可以在普�?Java 项目中使用�?

 ### [#](#ioc和aop是通过什么机制来实现�? IOC和AOP是通过什么机制来实现�? > Spring IOC 实现机制
 ...
 Spring AOP的实现依赖于**动态代理技�?*。动态代理是在运行时动态生成代理对象，而不是在编译时。它允许开发者在运行时指定要代理的接口和行为，从而实现在不修改源码的情况下增强方法的功能�?

 Spring AOP支持两种动态代理：

 - **基于JDK的动态代�?*：使用java.lang.reflect.Proxy类和java.lang.reflect.InvocationHandler接口实现。这种方式需要代理的类实现一个或多个接口�?
 - **基于CGLIB的动态代�?*：当被代理的类没有实现接口时，Spring会使用CGLIB库生成一个被代理类的子类作为代理。CGLIB（Code Generation Library）是一个第三方代码生成库，通过继承方式实现代理�?
 ### [#](#怎么理解springioc) 怎么理解SpringIoc�?**IOC**：Inversion Of Control，即控制反转，是一种设计思想。在传统�?Java SE 程序设计中，我们直接在对象内部通过 new 的方式来创建对象，是程序主动创建依赖对象�?

 ![image](https://cdn.xiaolincoding.com//picgo/1716790809843-e520e960-fb95-4511-aa30-73966361320a.webp)

 而在Spring程序设计中，IOC 是有专门的容器去控制对象�?

 ![image](https://cdn.xiaolincoding.com//picgo/1716790809860-74256f8b-3a96-485c-8aa1-11fa5dfb7640.webp)

 **所谓控�?*就是对象的创建、初始化、销毁�?

 - 创建对象：原来是 new 一个，现在是由 Spring 容器创建�?
 - 初始化对象：原来是对象自己通过构造器或�?setter 方法给依赖的对象赋值，现在是由 Spring 容器自动注入�?
 - 销毁对象：原来是直接给对象赋�?null 或做一些销毁操作，现在�?Spring 容器管理生命周期负责销毁对象�?
 总结：IOC 解决了繁琐的对象生命周期的操作，解耦了我们的代码�?*所谓反�?*：其实是反转的控制权，前面提到是�?Spring 来控制对象的生命周期，那么对象的控制就完全脱离了我们的控制，控制权交给了 Spring 。这个反转是指：我们由对象的控制者变成了 IOC 的被动控制者�?

 ### [#](#依赖倒置-依赖注入-控制反转分别是什�? 依赖倒置，依赖注入，控制反转分别是什么？ - 控制反转：“控制”指的是对程序执行流程的控制，而“反转”指的是在没有使用框架之前，程序员自己控制整个程序的执行。在使用框架之后，整个程序的执行流程通过框架来控制。流程的控制权从程序员“反转”给了框架�?
 ...
 ### [#](#依赖注入了解�?怎么实现依赖注入�? 依赖注入了解吗？怎么实现依赖注入的？ 在传统编程中，当一个类需要使用另一个类的对象时，通常会在该类内部通过`new`关键字来创建依赖对象，这使得类与类之间的耦合度较高�?

 而依赖注入则是将对象的创建和依赖关系的管理交�?Spring 容器来完成，类只需要声明自己所依赖的对象，容器会在运行时将这些依赖对象注入到类中，从而降低了类与类之间的耦合度，提高了代码的可维护性和可测试性�?
...
```
### [#](#如果让你设计一个springioc-你觉得会从哪些方面考虑这个设计) 如果让你设计一个SpringIoc，你觉得会从哪些方面考虑这个设计�?- Bean的生命周期管理：需要设计Bean的创建、初始化、销毁等生命周期管理机制，可以考虑使用工厂模式和单例模式来实现�?
 ...
 ### [#](#springaop主要想解决什么问�? SpringAOP主要想解决什么问�?它的目的是对于面向对象思维的一种补充，而不是像引入命令式、函数式编程思维让他顺应另一种开发场景。在我个人的理解下AOP更像是一种对于不支持多继承的弥补，除开对象的主要特征（我更喜欢叫“强共性”）被抽象为了一条继承链路，对于一些“弱共性”，AOP可以统一对他们进行抽象和集中处理�?
 ...
 ![image](https://cdn.xiaolincoding.com//picgo/1716791217083-132fe2ba-706a-4d8d-aef3-1617a2046585.png)

 ### [#](#aop在spring中的应用-你知道哪�? AOP在spring中的应用，你知道哪些�?�?Spring 中，AOP（面向切面编程）是核心特性之一，它通过分离横切关注点（如日志、事务、安全等）与业务逻辑，让代码更清晰、可维护。实际应用中，Spring AOP 的典型场景有这么几个�?
 ...
 - 然后�?*日志记录**，比如对接口的调用参数、返回结果、执行时间做日志，用 AOP 可以把这些逻辑抽离出来，定义一个切面，通过`@Before`获取入参，`@AfterReturning`获取返回值，`@Around`统计执行时间，这样业务方法里就不用掺杂日志代码，既干净又方便统一修改�?
 - 还有**权限校验**，比如某些接口需要登录后才能访问，或者需要特定角色权限。可以定义一个切面，在方法执行前（`@Before`）检查用户的登录状态或权限，如果不满足就直接抛出异常阻止方法执行，避免在每个接口方法里重复写权限判断�?
 Spring AOP 的这些应用，本质上都是通过 “切面�?封装横切逻辑，通过 “切入点�?指定要作用的方法，然后在方法执行的不同阶段（前、后、异常时等）插入这些逻辑，实现了 “无侵入�?地增强业务功能，这也�?AOP �?Spring 中如此重要的原因�?

 ### [#](#谈谈你对-spring-aop-的了解程�? 谈谈你对 Spring AOP 的了解程度？ 我对 Spring AOP 的理解，核心就是它是一�?*不侵入业务代码的横切逻辑解决方案**。比如日志、事务、权限校验这些，如果全写在业务方法里会非常乱，AOP 的作用就是把这些和业务无关、但又必须要做的事情抽出来，统一处理�?

 �?Spring 里，AOP 主要是通过**代理机制**来实现的。默认情况下，如果目标类实现了接口，Spring 用的�?JDK 动态代理；如果没有接口，就�?CGLIB 生成子类代理。所以本质上，调用的并不是原始对象，而是代理对象，方法执行前后会被织入额外逻辑�?

 ![image](https://cdn.xiaolincoding.com//picgo/1766813478098-9f0e495e-025c-4c6a-a1aa-7fbb133dbb8a.png)

 从使用角度来看，Spring AOP �?*基于运行时代理的 AOP**，不是像 AspectJ 那种编译期或类加载期织入。我们平时写 `@Aspect`、`@Before`、`@After`、`@Around` 这些注解，其实就是在定义切面、切点和通知，Spring 在容器启动的时候会解析这些配置，最后帮我们把代理对象准备好�?

 真正执行的时候，方法调用会走一条拦截器链。以 `@Around` 为例，本质上就是在方法执行前后包一层逻辑，是否放行、是否执行原方法，都是通过 `proceed()` 来控制的。事务、日志这些底层实现，其实就是一�?MethodInterceptor 在按顺序执行�?

 在实际项目里，我主要�?AOP 来做几类事情，比如声明式事务、接口日志、统一异常处理、参数校验或者一些轻量级的权限控制。这些场景的共同点是，它们都不属于核心业务逻辑，但又需要在很多地方重复出现，用 AOP 抽出来会非常干净�?

 当然 Spring AOP 也有它的边界。第一是它**只能作用�?Spring 容器管理�?Bean �?*，自�?new 出来的对象是不会生效的。第二是**同类内部方法调用不会触发 AOP**，因为走的是 `this`，绕过了代理。第三是它只能拦截方法级别的调用，做不到字段级别或者构造方法级别的增强�?

 整体来说，我会把 Spring AOP 当成一�?*工程层面的工�?*，用来解决通用、可复用的横切问题，而不是拿它去写复杂业务逻辑，超过这个边界我一般会考虑别的设计方式�?

 ### [#](#springaop的原理了解吗) SpringAOP的原理了解吗 Spring AOP的实现依赖于**动态代理技�?*。动态代理是在运行时动态生成代理对象，而不是在编译时。它允许开发者在运行时指定要代理的接口和行为，从而实现在不修改源码的情况下增强方法的功能�?

 Spring AOP支持两种动态代理：

 - **基于JDK的动态代�?*：使用java.lang.reflect.Proxy类和java.lang.reflect.InvocationHandler接口实现。这种方式需要代理的类实现一个或多个接口�?
 - **基于CGLIB的动态代�?*：当被代理的类没有实现接口时，Spring会使用CGLIB库生成一个被代理类的子类作为代理。CGLIB（Code Generation Library）是一个第三方代码生成库，通过继承方式实现代理�?
 ### [#](#动态代理是什�? 动态代理是什么？ Java的动态代理是一种在运行时动态创建代理对象的机制，主要用于在不修改原始类的情况下对方法调用进行拦截和增强�?
 ...
 ### [#](#动态代理和静态代理的区别) 动态代理和静态代理的区别 代理是一种常用的设计模式，目的是：为其他对象提供一个代理以控制对某个对象的访问，将两个类的关系解耦。代理类和委托类都要实现相同的接口，因为代理真正调用的是委托类的方法�?

 区别�?

 - 静态代理：由程序员创建或者是由特定工具创建，在代码编译时就确定了被代理的类是一个静态代理。静态代理通常只代理一个类�?
 - 动态代理：在代码运行期间，�?JVM 根据目标对象自动生成代理类，不需要为每个被代理类手写代理类。JDK 动态代理基于接口（要求目标对象实现接口），CGLIB 动态代理基于继承（通过生成目标类的子类作为代理，不要求目标对象实现接口）�?
 ### [#](#能使用静态代理的方式实现aop�? 能使用静态代理的方式实现AOP吗？ 当然**可以**用静态代理实�?AOP 的基本功能，比如你在代码里手动写个代理类，在目标方法前后加日志或者事务控制，技术上完全可行。但问题是，静态代理在实际生产中基本没人用，因为它有三大硬伤：

 - **第一是代码爆�?*。比如你�?100 �?Service 类需要加事务，就得写 100 个对应的静态代理类，里面全是重复的 `try-catch` 提交回滚代码，维护起来简直是灾难�?

 - **第二是僵�?*。一旦业务接口改了个方法名，所有相关的代理类都得跟着改，而动态代理通过反射调用目标方法，根本不怕这种变动�?

 - **第三是无法动态筛�?*。比如你想只给带 `@Transactional` 注解的方法加事务，静态代理只能写死逻辑，�?Spring AOP 可以在运行时通过切点表达式精准匹配需要增强的方法�?

 所�?Spring 才选了 JDK 动态代理和 CGLIB：它们能�?*运行时动态生成代理类**，一个切面配置就能覆盖成百上千个方法。像事务管理这种全局需求，用静态代理手动绑定根本不可行�?

 ### [#](#aop实现有哪些注�? AOP实现有哪些注解？ 常用的注解包括：
 ...
 ### [#](#什么是反射-有哪些使用场�? 什么是反射？有哪些使用场景�?反射机制是指程序在运行状态下，对于任意一个类，都能够获取这个类的所有属性和方法；对于任意一个对象，都能够调用它的任意属性和方法。也就是说，Java 反射允许在运行时获取类的信息并动态操作对象，即使在编译时不知道具体的类也能实现�?

 反射具有以下特性：

 - **运行时类信息访问**：反射机制允许程序在运行时获取类的完整结构信息，包括类名、包名、父类、实现的接口、构造函数、方法和字段等�?
 - **动态对象创�?*：可以使用反射API动态地创建对象实例，即使在编译时不知道具体的类名。这通常通过 `Constructor.newInstance()` 方法实现（`Class.newInstance()` �?Java 9 起已被标记为 `@Deprecated`，推荐使�?`clazz.getDeclaredConstructor().newInstance()`）�?
 - **动态方法调�?*：可以在运行时动态地调用对象的方法，包括私有方法。这通过Method类的invoke()方法实现，允许你传入对象实例和参数值来执行方法�?
 - **访问和修改字段�?*：反射还允许程序在运行时访问和修改对象的字段值，即使是私有的。这是通过Field类的get()和set()方法完成的�?
 ![image](https://cdn.xiaolincoding.com//picgo/1718957173277-863d2ec6-a754-423b-9066-9f28610d1a31-20240725232406027.png)
 ...
 ### [#](#spring是如何解决循环依赖的) **spring是如何解决循环依赖的�?* 循环依赖指的是两个类中的属性相互依赖对方：例如 A 类中�?B 属性，B 类中�?A属性，从而形成了一个依赖闭环，如下图�?

 ![image](https://cdn.xiaolincoding.com//picgo/1720684589425-d06727ba-bf03-484a-b656-c225554b5b49.png)
 ...
 只有【第三种方式】的循环依赖问题�?Spring 解决了，其他两种方式在遇到循环依赖问题时，Spring都会产生异常�?
 ...
 - **一级缓�?*（`singletonObjects`）最终交付完整Bean�?
 整个机制通过**中断初始化流程、逆向注入半成品、延迟代理生�?*三大策略，将循环依赖的死结转化为有序的接力协作�?

 值得注意的是，此方案仅适用于Setter/Field注入�?*单例Bean**；构造器注入因必须在实例化前获得依赖，仍会导致无解的死锁�?

 ### [#](#spring为什么用3级缓存解决循环依赖问�?�?级缓存不行吗) Spring为什么用3级缓存解决循环依赖问题？�?级缓存不行吗�?Spring 必须用三级缓存解决循环依赖，核心是为�?*正确处理需�?AOP 代理�?Bean**。如果只用二级缓存，会导致注入的对象形态错误，甚至破坏单例原则�?
 ...
 - **模板方法模式** : Spring �?jdbcTemplate、hibernateTemplate 等以 Template 结尾的对数据库操作的类，它们就使用到了模板模式�?
 - **包装器设计模�?* : 我们的项目需要连接多个数据库，而且不同的客户在每次访问中根据需要会去访问不同的数据库。这种模式让我们可以根据客户的需求能够动态切换不同的数据源�?
 - **观察者模�?** Spring 事件驱动模型就是观察者模式很经典的一个应用�?
 - **适配器模�?* :Spring AOP 的增强或通知(Advice)使用到了适配器模式、spring MVC 中也是用到了适配器模式适配Controller�?
 ### [#](#spring-常用注解有什�? spring **常用注解有什么？** > @Autowired 注解
...
在上面的示例代码中，MyRepository类被@Repository注解标记，Spring会将其实例化为一个bean，并将其添加到Spring容器中�?
 ...
 @Controller注解用于标记一个类作为控制层的组件。它也是@Component注解的特例，用于标记控制层的bean。这是MVC结构的另一个部分，加在控制�?
...
在上面的示例代码中，MyController类被@Controller注解标记，Spring会将其实例化为一个bean，并将其添加到Spring容器中�?
 ...
 ### [#](#spring的事务什么情况下会失�? Spring的事务什么情况下会失效？ Spring Boot通过Spring框架的事务管理模块来支持事务操作。事务管理在Spring Boot中通常是通过 @Transactional 注解来实现的。事务可能会失效的一些常见情况包�?

 - **异常�?try-catch 吞掉**：如果事务方法内部用 `try-catch` 捕获了异常却没有再抛出，Spring 的代理感知不到异常，事务会正常提交，回滚失效�?
 - **抛出的是受检异常（Checked Exception�?*：Spring 默认只对 `RuntimeException` �?`Error` 进行回滚，受检异常（如 `IOException`、`SQLException`）默认不会触发回滚，需要通过 `@Transactional(rollbackFor = Exception.class)` 显式声明�?
 - **事务传播属性设置不�?*: 如果在多个事务之间存在事务嵌套，且事务传播属性配置不正确，可能导致事务失效。特别是在方法内部调用有 @Transactional 注解的方法时要特别注意�?
 - **多数据源的事务管�?*: 如果在使用多数据源时，事务管理没有正确配置或者存在多�?@Transactional 注解时，可能会导致事务失效�?
 - **同类内部方法调用（this 调用�?*：如果一个事务方法内部通过 `this` 调用另一个带 `@Transactional` 的方法，由于绕过了代理对象，事务注解会失效�?
 - **事务在非公开方法中失�?*: 如果 @Transactional 注解标注在私有方法上或者非 public 方法上，事务也会失效�?
 ### [#](#spring的事�?使用this调用是否生效) Spring的事务，使用this调用是否生效�?不能生效�?

 因为Spring事务是通过代理对象来控制的，只有通过代理对象的方法调用才会应用事务管理的相关规则。当使用`this`直接调用时，是绕过了Spring的代理机制，因此不会应用事务设置�?

 ### [#](#bean的生命周期说一�? Bean的生命周期说一下？ ![image](https://cdn.xiaolincoding.com//picgo/1719570477922-ad595a67-be98-4272-9e13-8ad73dd75c13.png)
 ...
 - 容器关闭时，先调�?`@PreDestroy` 标注的方法（同样�?`CommonAnnotationBeanPostProcessor` 触发）；然后，如�?bean 实现�?`DisposableBean` 接口，Spring 将调用其 `destroy()` 方法；最后，如果 bean 使用�?`destroy-method` 声明销毁方法，该方法也会被调用�?
 ### [#](#bean是否单例) Bean是否单例�?Spring 中的 Bean 默认都是单例的�?
 ...
 ### [#](#bean的单例和非单�?生命周期是否一�? Bean的单例和非单例，生命周期是否一�?不一样的，Spring Bean 的生命周期完全由 IoC 容器控制。Spring 只帮我们管理单例模式 Bean 的完整生命周期，对于 `prototype` �?Bean，Spring 在创建好交给使用者之后，则不会再管理后续的生命周期�?
...
**适用场景** | 无状态服务（如Service、DAO层）�?| 有状态对象（如用户会话、临时计算对象）�?| ### [#](#spring-bean的作用域有哪�? Spring bean的作用域有哪些？ Spring框架中的Bean作用域（Scope）定义了Bean的生命周期和可见性。不同的作用域影响着Spring容器如何管理这些Bean的实例，包括它们如何被创建、如何被销毁以及它们是否可以被多个用户共享�?
 ...
 - **Singleton（单例）**：在整个应用程序中只存在一�?Bean 实例。默认作用域，Spring 容器中只会创建一�?Bean 实例，并在容器的整个生命周期中共享该实例�?
 - **Prototype（原型）**：每次请求时都会创建一个新�?Bean 实例。每次从容器中获取该 Bean 时都会创建一个新实例，适用于状态非常瞬时的 Bean�?
 - **Request（请求）**：每�?HTTP 请求都会创建一个新�?Bean 实例。仅�?Spring Web 应用程序中有效，每个 HTTP 请求都会创建一个新�?Bean 实例，适用�?Web 应用中需求局部性的 Bean�?
 - **Session（会话）**：Session 范围内只会创建一�?Bean 实例。该 Bean 实例在用户会话范围内共享，仅�?Spring Web 应用程序中有效，适用于与用户会话相关�?Bean�?
 - **Application**：当�?ServletContext 中只存在一�?Bean 实例。仅�?Spring Web 应用程序中有效，�?Bean 实例在整�?ServletContext 范围内共享，适用于应用程序范围内共享�?Bean�?
 - **WebSocket（Web套接字）**：在 WebSocket 范围内只存在一�?Bean 实例。仅在支�?WebSocket 的应用程序中有效，该 Bean 实例�?WebSocket 会话范围内共享，适用�?WebSocket 会话范围内共享的 Bean�?
 - **Custom scopes（自定义作用域）**：Spring 允许开发者定义自定义的作用域，通过实现 Scope 接口来创建新�?Bean 作用域�?
...
### [#](#spring容器里存的是什�? Spring容器里存的是什么？ 在Spring容器中，存储�?*主要是Bean对象**�?

 Bean是Spring框架中的基本组件，用于表示应用程序中的各种对象。当应用程序启动时，Spring容器会根据配置文件或注解的方式创建和管理这些Bean对象。Spring容器会负责创建、初始化、注入依赖以及销毁Bean对象�?

 ### [#](#在spring�?在bean加载-销毁前�?如果想实现某些逻辑-可以怎么�? 在Spring中，在bean加载/销毁前后，如果想实现某些逻辑，可以怎么�?在Spring框架中，如果你希望在Bean加载（即实例化、属性赋值、初始化等过程完成后）或销毁前后执行某些逻辑，你可以使用Spring的生命周期回调接口或注解。这些接口和注解允许你定义在Bean生命周期的关键点执行的代码�?
...
> 使用@Bean注解的initMethod和destroyMethod属�?
...
### [#](#spring给我们提供了很多扩展�?这些有了解吗) Spring给我们提供了很多扩展点，这些有了解吗�?Spring框架提供了许多扩展点，使得开发者可以根据需求定制和扩展Spring的功能。以下是一些常用的扩展点：
 ...
 - **服务�?Controller 控制器接收到请求后对请求进行解析，找到相应的Model，对用户请求进行处理Model 处理�?*
 - **将处理结果再交给 Controller（控制器其实只是起到了承上启下的作用）；**
 - **根据处理结果找到要作为向客户端发回的响应View 页面，页面经渲染后发送给客户端�?*
 ### [#](#了解springmvc的处理流程吗) 了解SpringMVC的处理流程吗�?![image](https://cdn.xiaolincoding.com//picgo/1716791047520-ac0d9673-be0a-4005-8732-30bdedc8f1af.webp)
 ...
 - DispatcherServlet根据处理器Handler获取处理器适配器HandlerAdapter执行HandlerAdapter处理一系列的操作，如：参数封装，数据格式转换，数据验证等操�?
 ...
 ### [#](#handlermapping-�?handleradapter有了解吗) Handlermapping �?handleradapter有了解吗�?HandlerMapping�?
 ...
 - **作用**：HandlerAdapter负责调用处理�?Controller)来处理请求�?
 ...
 ## [#](#springboot) SpringBoot ### [#](#为什么使用springboot) 为什么使用springboot - 简化开发：Spring Boot通过提供一系列的开箱即用的组件和自动配置，简化了项目的配置和开发过程，开发人员可以更专注于业务逻辑的实现，而不需要花费过多时间在繁琐的配置上�?
 ...
 ### [#](#springboot比spring好在哪里) SpringBoot比Spring好在哪里 - Spring Boot 提供了自动化配置，大大简化了项目的配置过程。通过约定优于配置的原则，很多常用的配置可以自动完成，开发者可以专注于业务逻辑的实现�?
 - Spring Boot 提供了快速的项目启动器，通过引入不同�?Starter，可以快速集成常用的框架和库（如数据库、消息队列、Web 开发等），极大地提高了开发效率�?
 - Spring Boot 默认集成了多种内嵌服务器（如Tomcat、Jetty、Undertow），无需额外配置，即可将应用打包成可执行�?JAR 文件，方便部署和运行�?
 ### [#](#springboot用到哪些设计模式) SpringBoot用到哪些设计模式�?- **代理模式**：Spring �?AOP 通过动态代理实现方法级别的切面增强，有静态和动态两种代理方式，采用动态代理方式�?
 - **策略模式**：Spring AOP 支持 JDK �?Cglib 两种动态代理实现方式，通过策略接口和不同策略类，运行时动态选择，其创建一般通过工厂方法实现�?
 - **装饰器模�?*：Spring �?TransactionAwareCacheDecorator 解决缓存与数据库事务问题增加对事务的支持�?
 - **单例模式**：Spring Bean 默认是单例模式，通过单例注册表（�?HashMap）实现�?
 - **简单工厂模�?*：Spring 中的 BeanFactory 是简单工厂模式的体现，通过工厂类方法获�?Bean 实例�?
 - **工厂方法模式**：Spring中的 FactoryBean 体现工厂方法模式，为不同产品提供不同工厂�?
 - **观察者模�?*：Spring 观察者模式包�?Event 事件、Listener 监听者、Publisher 发送者，通过定义事件、监听器和发送者实现，观察者注册在 ApplicationContext 中，消息发送由 ApplicationEventMulticaster 完成�?
 - **模板模式**：Spring Bean 的创建过程涉及模板模式，体现扩展性，类似 Callback 回调实现方式�?
 - **适配器模�?*：Spring MVC 中针对不同方式定义的 Controller，利用适配器模式统一函数定义，定义了统一接口 HandlerAdapter 及对应适配器类�?
 ### [#](#怎么理解springboot中的约定大于配置) 怎么理解SpringBoot中的约定大于配置 约定大于配置是Spring Boot的核心设计理念，它通过**预设合理的默认行为和项目规范**，大幅减少开发者需要手动配置的步骤，从而提升开发效率和项目标准化程度�?

 理解 Spring Boot 中的“约定大于配置”原则，可以从以下几个方面来解释�?

 - **自动化配�?*：Spring Boot 提供了大量的自动化配置，通过分析项目的依赖和环境，自动配置应用程序的行为。开发者无需显式地配置每个细节，大部分常用的配置都已经预设好了。例如，引入`spring-boot-starter-web`后，Spring Boot会自动配置内嵌Tomcat和Spring MVC，无需手动编写XML�?
 - **默认配置**：Spring Boot 为诸多方面提供大量默认配置，如连接数据库、设�?Web 服务器、处理日志等。开发人员无需手动配置这些常见内容，框架已做好决策。例如，默认的日志配置可让应用程序快速输出日志信息，无需开发者额外繁琐配置日志级别、输出格式与位置等�?
 - **约定的项目结�?*：Spring Boot 提倡特定项目结构，通常主应用程序类（含 main 方法）置于根包，控制器类、服务类、数据访问类等分别放在相应子包，如`com.example.demo.controller`放控制器类，`com.example.demo.service`放服务类等。此约定使团队成员更易理解项目结构与组织，新成员加入项目时能快速定位各功能代码位置，提升协作效率�?
 ### [#](#springboot的项目结构是怎么样的) SpringBoot的项目结构是怎么样的�?一个正常的企业项目里一种通用的项目结构和代码层级划分的指导意见。按这《阿里巴巴Java开发手册》时本书上说的，一般分为如下几层：

 ![image](https://cdn.xiaolincoding.com//picgo/1721712085278-f25ffdd1-26bc-4a7c-928a-42e48501b285.png)

 - 开放接口层：可直接封装 Service 接口暴露�?RPC 接口；通过 Web 封装�?http 接口；网关控制层等�?
 ...
 - 3）与 DAO 层交互，对多�?DAO 的组合复用�?

 - DAO 层：数据访问层，与底�?MySQL、Oracle、Hbase、OceanBase 等进行数据交互�?
 ...
 ![image](https://cdn.xiaolincoding.com//picgo/1721712159282-79195670-9acf-4bfb-93b1-47d089a4bc1c.png)

 对应代码目录的流转逻辑就是�?

 ![image](https://cdn.xiaolincoding.com//picgo/1721712166152-4774fdac-cb1e-4fb1-929e-3ae130145aa8.png)

 所以，以后每当我们拿到一个新的项目到手时，只要按照这个思路去看别人项目的代码，应该基本都是能理得顺的�?

 ### [#](#springboot自动装配原理是什�? SpringBoot自动装配原理是什么？ > 什么是自动装配�?
 ...
 首先点进 `@SpringBootApplication` 注解的内�?

 ![image](https://cdn.xiaolincoding.com//picgo/1719493065090-4ce24f82-8f39-4aa6-b399-671e68e9c18f.png)

 接下来将逐个解释这些注解的作用：

 - `@Target({ElementType.TYPE})`: 该注解指定了这个注解可以用来标记在类上。在这个特定的例子中，这表示该注解用于标记配置类�?
 - `@Retention(RetentionPolicy.RUNTIME)`: 这个注解指定了注解的生命周期，即在运行时保留。这是因�?Spring Boot 在运行时扫描类路径上的注解来实现自动配置，所以这里使用了 RUNTIME 保留策略�?
 - `@Documented`: 该注解表示这个注解应该被包含�?Java 文档中。它是用于生成文档的标记，使开发者能够看到这个注解的相关信息�?
 - `@Inherited`: 这个注解指示一个被标注的类型是被继承的。在这个例子中，它表明这个注解可以被继承，如果一个类继承了带有这个注解的类，它也会继承这个注解�?
 - `@SpringBootConfiguration`: 这个注解表明这是一�?Spring Boot 配置类。如果点进这个注解内部会发现与标准的 @Configuration 没啥区别，只是为了表明这是一个专门用�?SpringBoot 的配置�?
 - `@EnableAutoConfiguration`: 这个注解�?Spring Boot 自动装配的核心。它告诉 Spring Boot 启用自动配置机制，根据项目的依赖和配置自动配置应用程序的上下文。通过这个注解，SpringBoot 将尝试根据类路径上的依赖自动配置应用程序�?
 - `@ComponentScan`: 这个注解用于配置组件扫描的规则。在这里，它告诉 SpringBoot 在指定的包及其子包中查找组件，这些组件包括被注解的类、@Component 注解的类等。其中的 excludeFilters 参数用于指定排除哪些组件，这里使用了两个自定义的过滤器，分别�?TypeExcludeFilter �?AutoConfigurationExcludeFilter�?
...
public class AutoConfigurationImportSelector implements DeferredImportSelector, BeanClassLoaderAware,
 ...
 // 获取所有符合条件的类的全限定类名，例如RedisTemplate的全限定类名(org.springframework.data.redis.core.RedisTemplate;)，这些类需要被加载�?IoC 容器中�?
	@Override
	public String[] selectImports(AnnotationMetadata annotationMetadata) {
		// 扫描类路径上 META-INF/spring.factories（或 2.7+ �?AutoConfiguration.imports）文件，
		...
		// 排序自动配置类，根据 @AutoConfigureOrder �?@AutoConfigureAfter/@AutoConfigureBefore 注解指定的顺�?
		sort(configurations, annotationMetadata, attributes);

		// 将满足条件的自动配置类的类名数组返回，这些类将被导入到应用程序上下文�?
		return StringUtils.toStringArray(configurations);
	}

	// ... （其他方法）
	protected List&lt;String&gt; getCandidateConfigurations(AnnotationMetadata metadata, AnnotationAttributes attributes) {
		// 获取自动配置类的候选列表，�?META-INF/spring.factories 文件中读�?
		// 通过类加载器加载所有候选类
		List&lt;String&gt; configurations = SpringFactoriesLoader.loadFactoryNames(getSpringFactoriesLoaderFactoryClass(),
				getBeanClassLoader());

		// 根据 spring.boot.enableautoconfiguration 属性过�?
		configurations = configurations.stream()
				.filter(this::isEnabled)
				.collect(Collectors.toList());

		// 对于 Spring Boot 1.x 版本，还需要添�?spring-boot-autoconfigure 包中的自动配置类
	...
	protected void sort(List&lt;String&gt; configurations, AnnotationMetadata metadata,
			AnnotationAttributes attributes) {
		// 根据 @AutoConfigureOrder �?@AutoConfigureAfter/@AutoConfigureBefore 注解指定的顺序对自动配置类进行排�?
...
梳理一下，以下是`AutoConfigurationImportSelector`的主要工作：
 ...
 - 根据条件导入自动配置�? 满足条件的自动配置类将被导入到应用程序的上下文中。这意味着它们会被实例化并应用于应用程序的配置�?
...
> 步骤2: 添加自动配置
 ...
 添加自动配置的元数据。在 Spring Boot 2.7 之前，放�?`src/main/resources/META-INF/spring.factories` 中：
...
my:
 name: Hello World
### [#](#springboot里面有哪些重要的注解-还有一个配置相关的注解是哪�? SpringBoot里面有哪些重要的注解？还有一个配置相关的注解是哪个？ Spring Boot 中一些常用的注解包括�?
 ...
 ### [#](#springboot怎么开启事�? springboot怎么开启事务？ �?Spring Boot 中开启事务非常简单，只需在服务层的方法上添加 `@Transactional` 注解即可�?

 例如，假设我们有一�?UserService 接口，其中有一个保存用户的方法 saveUser()�?

 
public interface UserService {
 void saveUser(User user);
}
我们希望在这个方法中开启事务，只需在该方法上添�?`@Transactional` 注解，如下所示：
...
&lt;/dependency&gt;
### [#](#springboot怎么做到导入就可以直接使用的) Springboot怎么做到导入就可以直接使用的�?这个主要依赖于自动配置、起步依赖和条件注解等特性�?

 > 起步依赖

 起步依赖是一种特殊的 Maven �?Gradle 依赖，它将项目所需的一系列依赖打包在一起。例如，`spring-boot-starter-web` 这个起步依赖就包含了 Spring Web MVC、Tomcat 等构�?Web 应用所需的核心依赖�?

 开发者只需在项目中添加一个起步依赖，Maven �?Gradle 就会自动下载并管理与之关联的所有依赖，避免了手动添加大量依赖的繁琐过程�?

 比如，在 `pom.xml` 中添�?`spring-boot-starter-web` 依赖�?
...
> 自动配置
 ...
 开发者可以通过自定义配置来覆盖自动配置的默认行为。如果开发者在 `application.properties` �?`application.yml` 中定义了特定的配置，或者在代码中定义了同名�?Bean，Spring Boot 会优先使用开发者的配置�?

 > 条件注解

 条件注解用于控制 Bean 的创建和加载，只有在满足特定条件时，才会创建相应�?Bean。Spring Boot 的自动配置类中广泛使用了条件注解，如 `@ConditionalOnClass`、`@ConditionalOnMissingBean` 等�?

 比如，`@ConditionalOnClass` 表示只有当类路径中存在指定的类时，才会创建该 Bean。例如，�?`WebMvcAutoConfiguration` 类中，可能会有如下代码：
...
这段代码表示只有当类路径中存�?`Servlet`、`DispatcherServlet` �?`WebMvcConfigurer` 类时，才会加�?`WebMvcAutoConfiguration` 类中的配置�?

 ### [#](#springboot-过滤器和拦截器说一�? SpringBoot 过滤器和拦截器说一下？ �?Spring Boot 中，过滤器（Filter）和拦截器（Interceptor）是用于处理请求和响应的两种不同机制�?

 **
**特�?*** | ****过滤器（Filter�?*** | ****拦截器（Interceptor�?*** | 
**规范/框架** | Servlet 规范（Spring Boot 2.x �?`javax.servlet.Filter`；Spring Boot 3.x 起改�?`jakarta.servlet.Filter`�?| Spring MVC 框架（`org.springframework.web.servlet.HandlerInterceptor`�?| 
...
**适用场景** | 全局请求处理（编码、日志、安全） | 业务逻辑相关的处理（权限、参数校验） | 过滤器是 Java Servlet 规范中的一部分，它可以对进�?Servlet 容器的请求和响应进行预处理和后处理。过滤器通过实现 `javax.servlet.Filter`（Spring Boot 3.x 起为 `jakarta.servlet.Filter`）接口，并重写其中的 `init`、`doFilter` �?`destroy` 方法来完成相应的逻辑。当请求进入 Servlet 容器时，会按照配置的顺序依次经过各个过滤器，然后再到达目�?Servlet 或控制器；响应返回时，也会按照相反的顺序再次经过这些过滤器�?
 ...
 - �?JDBC 相比，减少了 50%以上的代码量，消除了 JDBC 大量冗余的代码，�?需要手动开关连接；
 - 很好的与各种数据库兼容，因为 MyBatis 使用 JDBC 来连接数据库，所以只�?JDBC 支持的数据库 MyBatis 都支持�?
 - 能够�?Spring 很好的集成，开发效率高
 - 提供映射标签，支持对象与数据库的 ORM 字段关系映射；提供对象关系映�?标签，支持对象关系组件维护�?
 ### [#](#mybatis觉得在哪方面做的比较�? MyBatis觉得在哪方面做的比较好？ MyBatis �?**SQL 灵活�?*�?*动�?SQL 支持**�?*结果集映�?*�?*�?Spring 整合**方面表现卓越，尤其适合重视 SQL 可控性的项目�?

 - SQL 与代码解耦，灵活可控：MyBatis 允许开发者直接编写和优化 SQL，相比全自动 ORM（如 Hibernate），MyBatis 让开发者明确知道每�?SQL 的执行逻辑，便于性能调优�?
...
&lt;select id="findUserWithRole" resultMap="userRoleMap"&gt;
 ...
 WHERE u.id = #{userId}
&lt;/select&gt;
- 动�?SQL 的强大支持：比如可以动态拼接SQL，通过 `&lt;if&gt;`, `&lt;choose&gt;`, `&lt;foreach&gt;` 等标签动态生�?SQL，避�?Java 代码中繁琐的字符串拼接�?
...
```
- �?Spring 生态无缝集成：通过 `@MapperScan` 快速扫�?Mapper 接口，结�?Spring 事务管理，配置简洁高效�?
...
### [#](#还记得jdbc连接数据库的步骤�? 还记得JDBC连接数据库的步骤吗？ 使用Java JDBC连接数据库的一般步骤如下：
 ...
 - **处理查询结果**：如果是 SELECT 查询操作，通过 ResultSet 对象来处理查询结果。可以使�?ResultSet �?next() 方法遍历查询结果集，然后通过 getXXX() 方法获取各个字段的值�?
 - **关闭连接**：在完成数据库操作后，需要逐级关闭数据库连接相关对象，即先关闭 ResultSet，再关闭 Statement，最后关�?Connection�?
 以下是一个简单的示例代码�?
...
请注意，在实际应用中，需要进行异常处理以确保资源的正确释放，以及使用 try-with-resources 来简化代码和确保资源的及时关闭�?
 ...
 ### [#](#如果项目中要用到原生的mybatis去查�?该怎样�? 如果项目中要用到原生的mybatis去查询，该怎样写？ 步骤概述�?
...
通过以上步骤，你可以利用原生的MyBatis框架来进行数据库查询操作。请确保配置正确、SQL语句准确并与数据库字段匹配，以确保查询的准确性和高效性�?
 ...
 ### [#](#mybatis里的-�?的区�? Mybatis里的 # �?$ 的区别？ - Mybatis 在处�?#{} 时，会创建预编译�?SQL 语句，将 SQL 中的 #{} 替换�?? 号，在执�?SQL 时会为预编译 SQL 中的占位符（?）赋值，调用 PreparedStatement �?set 方法来赋值，预编译的 SQL 语句执行效率高，并且可以防止SQL 注入，提供更高的安全性，适合传递参数值�?
 - Mybatis 在处�?${} 时，只是创建普通的 SQL 语句，然后在执行 SQL 语句�?MyBatis 将参数直接拼入到 SQL 里，不能防止 SQL 注入，因为参数直接拼接到 SQL 语句中，如果参数未经过验证、过滤，可能会导致安全问题�?
 ### [#](#mybatisplus和mybatis的区�? MybatisPlus和Mybatis的区别？ MybatisPlus是一个基于MyBatis的增强工具库，旨在简化开发并提高效率。以下是MybatisPlus和MyBatis之间的一些主要区别：

 - **CRUD操作**：MybatisPlus通过继承BaseMapper接口，提供了一系列内置的快捷方法，使得CRUD操作更加简单，无需编写重复的SQL语句�?
 - **代码生成�?*：MybatisPlus提供了代码生成器功能，可以根据数据库表结构自动生成实体类、Mapper接口以及XML映射文件，减少了手动编写的工作量�?
 ...
 - **多租户支�?*：MybatisPlus提供了多租户的支持，可以轻松实现多租户数据隔离的功能�?
 - **注解支持**：MybatisPlus引入了更多的注解支持，使得开发者可以通过注解来配置实体与数据库表之间的映射关系，减少了XML配置文件的编写�?
 ### [#](#mybatis运用了哪些常见的设计模式) MyBatis运用了哪些常见的设计模式�?- 建造者模式（Builder），如：SqlSessionFactoryBuilder、XMLConfigBuilder、XMLMapperBuilder、XMLStatementBuilder、CacheBuilder等；
 ...
 ## [#](#springcloud) SpringCloud ### [#](#了解springcloud�?说一下他和springboot的区�? 了解SpringCloud吗，说一下他和SpringBoot的区�?Spring Boot是用于构建单个Spring应用的框架，而Spring Cloud则是用于构建分布式系统中的微服务架构的工具，Spring Cloud提供了服务注册与发现、负载均衡、断路器、网关等功能�?

 两者可以结合使用，通过Spring Boot构建微服务应用，然后用Spring Cloud来实现微服务架构中的各种功能�?

 ### [#](#用过哪些微服务组�? 用过哪些微服务组件？ ![image](https://cdn.xiaolincoding.com//picgo/1715933382453-968d55a3-059b-423d-9f67-0ebf732fc400.png)
 ...
 ![image](https://cdn.xiaolincoding.com//picgo/1715933831252-4dbe4ae0-8072-4512-b54b-39ea5e8b3153.png)

 - SpringCloud Alibaba中使�?*Alibaba Nacos**组件实现**注册中心**，Nacos提供了一组简单易用的特性集，可快速实现动态服务发现、服务配置、服务元数据及流量管理�?
 - SpringCloud Alibaba 早期复用 Ribbon 做客户端负载均衡，新版本�?020.x+）已切换�?**Spring Cloud LoadBalancer**，并可结�?Nacos 提供的权重、健康状态等元数据实现加权负载均衡（Nacos 主要承担注册中心 / 配置中心角色，真正的负载均衡仍由客户端组件完成）�?
 - SpringCloud Alibaba 使用 **OpenFeign**（社区维护的 Feign 分支，Netflix Feign 已停更）�?**Alibaba Dubbo** 组件来实现服务通信，前者与 SpringCloud 采用了相同的方案，后者则是对自家�?**RPC 框架 Dubbo** 也给予支持，为服务间通信提供另一种选择�?
 - SpringCloud Alibaba �?*API服务网关**组件中，使用与SpringCloud相同的组件，即：**SpringCloud Gateway**�?
 ...
 - SpringCloud Alibaba�?*分布式链路组�?*中采用与SpringCloud相同的方案，即：**Sleuth/Zipkin Server**�?
 - SpringCloud Alibaba使用**Alibaba Sentinel**实现系统保护，Sentinel不仅功能更强大，实现系统保护比Hystrix更优雅，而且还拥有更好的UI界面�?
 ### [#](#负载均衡有哪些算�? 负载均衡有哪些算法？ - 简单轮询：将请求按顺序分发给后端服务器上，不关心服务器当前的状态，比如后端服务器的性能、当前的负载�?
 - 加权轮询：根据服务器自身的性能给服务器设置不同的权重，将请求按顺序和权重分发给后端服务器，可以让性能高的机器处理更多的请�?
 ...
 - 最小活跃数：统计每台服务器上当前正在处理的请求数，也就是请求活跃数，将请求分发给活跃数最少的后台服务�?
 ### [#](#如何实现一直均衡给一个用�? 如何实现一直均衡给一个用户？ 可以通过「一致性哈希算法」来实现，根据请求的客户�?ip、或请求参数通过哈希算法得到一个数值，利用该数值取模映射出对应的后端服务器，这样能保证同一个客户端或相同参数的请求每次都使用同一台服务器�?

 ### [#](#介绍一下服务熔�? 介绍一下服务熔�?服务熔断是应对微服务雪崩效应的一�?*链路保护机制，类似股市、保险丝**�?
...
---
 [![image](https://cdn.xiaolincoding.com/mianshiya.png)](https://mianshiya.com/?shareCode=xeu1wi)

 
---
 最新的图解文章都在公众号首发，别忘记关注哦！！如果你想加入百人技术交流群，扫码下方二维码回复「加群」�?

 ![image](https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost3@main/%E5%85%B6%E4%BB%96/%E5%85%AC%E4%BC%97%E5%8F%B7%E4%BB%8B%E7%BB%8D.png)

---
> 参考来源：[Spring面试题](https://xiaolincoding.com/interview/spring.html)
