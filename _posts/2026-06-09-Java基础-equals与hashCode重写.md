...
1. **一致性**：多次调用同一对象的 `hashCode()`，必须返回相同的值
2. **相等则哈希必等**：如果 `equals()` 返回 `true`，`hashCode()` 必须返回相同的值
3. **哈希等不一定相等**：`hashCode()` 相同，`equals()` 不一定返回 `true`（哈希冲突）
...
---
> 参考来源：[为什么重写 equals 时一定要重写 hashCode？](https://www.quanxiaoha.com/java-interview/java-equals-hashcode-zhongxie-why)
