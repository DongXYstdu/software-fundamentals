...
![TCP三次握手](https://cdn.xiaolincoding.com//mysql/other/8d55a06f2efa946921ff61a008c76b00.png)

服务端代码，对socket执行bind方法可以绑定监听端口，然后执行 `listen方法` 后，就会进入监听（ `LISTEN` ）状态。内核会为每一个处于 `LISTEN` 状态的 `socket` 分配两个队列，分别叫 **半连接队列和全连接队列** 。

![每个listen Socket都有一个全连接和半连接队列](https://cdn.xiaolincoding.com//mysql/other/d7e2d60b28b0f9b460aafbf1bd6e7892.png)
...
← [4.20 服务端没有 listen，客户端发起连接建立，会发生什么？](https://xiaolincoding.com/network/3_tcp/tcp_no_listen.html) [4.22 用了 TCP 协议，数据一定不会丢吗？](https://xiaolincoding.com/network/3_tcp/tcp_drop.html) →

<div class='context-nav'>
<a class='context-link prev' href='/software-fundamentals/posts/计算机网络-TCP-拔掉网线后连接还存在吗/'><span class='context-label'>上一篇</span><span class='context-title'>拔掉网线后，原本TCP 连接还存在吗</span></a>
<a class='context-link next' href='/software-fundamentals/posts/计算机网络-TCP-没有listen会发生什么/'><span class='context-label'>下一篇</span><span class='context-title'>服务端没listen，客户端发起连接建立，会发生什么？</span></a>
</div>
