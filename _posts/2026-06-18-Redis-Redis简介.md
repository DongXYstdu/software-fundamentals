...
在网上看到一篇很不错的 Redis 文章，内容很精炼，而且图画的非常棒，所以分享给大家一起拜读拜读，主要说了「 **Redis 介绍、Redis 架构、Redis 持久化** 」这三个方面。

算是把Redis核心的功能都介绍了一下，可以从宏观的视角来了解 Redis，很适合作为重新复习 Redis 的文章。

当然，这篇文章也不是把所有 Redis 功能都介绍了，如果你想了解更多 Redis 其他知识，可以看我之前写的这篇： [3 万字 + 40 张图 | 攻破 Redis 常见面试题](https://xiaolincoding.com/redis/base/redis_interview.html)
...
| 支持广泛的编程语言 | Yes | Yes |
| 高级数据结构 | - | Yes |
| 多线程架构 | Yes | Redis 6.0 起支持多线程 I/O（命令执行仍为单线程） |
| 快照 | - | Yes |
| 复制（Replication） | - | Yes |
| 事务 | - | Yes |
| 发布/订阅 | - | Yes |
...
<a class='context-link prev' href='/software-fundamentals/posts/Redis-Redis数据结构图解/'><span class='context-label'>上一篇</span><span class='context-title'>图解Redis数据结构</span></a>
<a class='context-link next' href='/software-fundamentals/posts/Redis-Redis集群/'><span class='context-label'>下一篇</span><span class='context-title'>为什么要有cluster集群</span></a>
</div>
