...
- 针对 **当前读** （select... for update 等语句），是 **通过 next-key lock（记录锁+间隙锁）方式解决了幻读** ，因为当执行 select... for update 语句的时候，会加上 next-key lock，如果有其他事务在 next-key lock 锁范围内插入了一条记录，那么这个插入语句就会被阻塞，无法成功插入，所以就很好地避免了幻读问题。
...
做个实验，数据库表 t\_stu 如下，其中 id 为主键。

![](https://cdn.xiaolincoding.com//mysql/other/7f9df142b3594daeaaca495abb7133f5.png)
...
![](https://cdn.xiaolincoding.com//mysql/other/1f872ff92b644b5f81cee2dd9188b199.png)
...
![](https://cdn.xiaolincoding.com//mysql/other/3af285a8e70f4d4198318057eb955520.png)
...
![](https://cdn.xiaolincoding.com//mysql/other/7f9df142b3594daeaaca495abb7133f5-20230309222119359.png)
...
- T3 时刻：事务 A 再执行「当前读语句」 select \* from t\_test where id > 100 for update 就会得到 4 条记录，此时也发生了幻读现象。
...
<a class='context-link prev' href='/software-fundamentals/posts/MySQL-单表不超过2000W行/'><span class='context-label'>上一篇</span><span class='context-title'>MySQL单表不要超过2000W行靠谱吗</span></a>
<a class='context-link next' href='/software-fundamentals/posts/MySQL-数据页角度看B+树/'><span class='context-label'>下一篇</span><span class='context-title'>从数据页的角度看B+</span></a>
</div>
